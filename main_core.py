...
async def on_message(message):
                ...
                if message.content.startswith("/공지"):
                    ...
                    if message.author.server_permissions.administrator:
                        ...
                        embed = discord.Embed(title=str(g) + "님의 공지 | 's notice", color=0x00ff00)
                        embed.add_field(name=learn, value="관리자 권한(Adminstrator role) : 인증(Certified)✅")
                        ...
                        await app.send_message(message.channel, "완료!(Sucessful!)")
                ...
                if message.content.startswith('/잠수'):
                    ...
                    afk.append(id)
                    if learn == '':
                        embed = discord.Embed(title="잠수 시작!(Afk is start!)", color=0x00ff00)
                        embed.add_field(name=(message.author.name) + "님의 잠수 상태가 시작되었습니다!!!('s afk is started!!!!)", value="잠수 시작 시간(Afk started time) : " + str(a) + "년 " + str(b) + "월 " + str(c) + "일 " + str(d) + "시 " + str(e) + "분")
                        await app.send_message(message.channel, embed=embed)
                    else:
                        embed = discord.Embed(title="잠수 시작!(Afk is start!)", color=0x00ff00)
                        embed.add_field(name=(message.author.name) + "님의 잠수 상태가 시작되었습니다!!!('s afk is started!!!)", value="잠수 시작 시간(Afk started time) : " + str(a) + "년 " + str(b) + "월 " + str(c) + "일 " + str(d) + "시 " + str(e) + "분")
                        embed.add_field(name="사유(Reason)", value=learn)
                        await app.send_message(message.channel, embed=embed)

                if message.content.startswith('/패치노트'):
                    embed = discord.Embed(title="업데이트 내역(update report)", color=0x00ff00)
                    embed.add_field(name="●전체 명령어 전면 영어화", value="Jenon Bot의 모든 명령어에 영어를 추가하였습니다.")
                    embed.add_field(name="●긴급수정:봇 업데이트", value="20200113 Update")
                    await app.send_message(message.channel, embed=embed)
